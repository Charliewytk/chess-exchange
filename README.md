...
- **LIVE BOARD markets (lichess TV)** — lichess's public streaming API serves its
  ...
  markets have a real-time board, live clocks, and prices that move with the
  position. This is the full "bet while you watch" experience, working out of
  the box.
- **LIVE markets (chess.com)** — the app polls chess.com's public top-games roster
  ...
  from an Elo + draw-rate prior. When the real game ends, the official result
...
- **REPLAY markets** — chess.com doesn't expose move-by-move data to anonymous
  ...
  trade on material and clock, so prices swing on blunders like the real thing.
...
- **Bots** — six liquidity bots with different aggression/noise/trust profiles
  trade toward their beliefs (Elo prior, and on replays material + clock + game
  progress). They never see the stored result of a replay.
...
- Draw modelling is a crude Elo-based prior; no engine eval anywhere.
